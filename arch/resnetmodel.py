...
# ResNet for Cifar architecture, with extra linear layer to allow us to take activations from relu
...
class ResNetModel(nn.Module):
  def __init__(self, block, num_block, num_classes):
    ...
    last_linear = [nn.Linear(512 * block.expansion * 4 * 4, 512 * block.expansion)] # 512 * block.expansion # 4096
    last_linear.append(nn.ReLU(inplace=True))
    last_linear.append(nn.Dropout())
    self.linear_feats = nn.Sequential(*last_linear)
  ...
  def forward(self, x):
    ...
    output = self.linear_feats(output)
    output = self.fc(output)

    return output


def resnet18model(num_classes):
...
def resnet101model(num_classes):
  """ return a ResNet 101 object
  """
  return ResNetModel(BottleNeck, [3, 4, 23, 3], num_classes)


def resnet152model(num_classes):
