...
# in_distribution _pre script needs to have been run first

template = "python -m scripts.out_of_distribution --new_distr_data %s --new_distr_data_root %s " \
           "--data %s --model %s --seed %d --cuda subfunctions --search_ps %s"

for data in datasets:
  for model, ps in models_ps:
    for ood_dataset, ood_path in ood_datasets[data]:
      for seed in seeds:
        cmd = template % (ood_dataset, ood_path, data, model, seed, ps)
